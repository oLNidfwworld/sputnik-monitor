...
const { createNewPoint } = pointStore; 

function createNewPointAndScroll( ) {
    createNewPoint();
}

const selectedPoints = shallowRef<number[]>([]);
const isSelectedAll = computed(() => filteredPoints.value.every(p => selectedPoints.value.includes(p.id)));
  ...
  <div class="app-wrapper">
    <div class="grid grid-cols-[3fr_5fr] grid-rows-1 h-full gap-5">
      <div class="flex flex-col">
        ...
        <div class="mb-3">
          <CustomButton @click="createNewPointAndScroll" class="w-full" theme="colored">
            Создать
          </CustomButton>
        </div>
